...
// 每个模型的 draft version（对齐参考项目）
const MODEL_DRAFT_VERSIONS: Record<string, string> = {
  "jimeng-5.0": "3.3.9",
  "jimeng-4.6": "3.3.9",
  "jimeng-4.5": "3.3.4",
  "jimeng-4.1": "3.3.4",
  ...
  "jimeng-xl-pro": "3.0.2",
  // jimeng-image-X.X 别名
  "jimeng-image-4.5": "3.3.4",
  "jimeng-image-4.1": "3.3.4",
  "jimeng-image-4.0": "3.3.4",
  "jimeng-image-3.1": "3.0.2",
  "jimeng-image-3.0": "3.0.2",
  "jimeng-image-2.0-pro": "3.0.2",
};

function getDraftVersion(model: string): string {
  return MODEL_DRAFT_VERSIONS[model] || "3.3.4";
}
...
const DIMENSIONS_4K: Record<string, { width: number; height: number }> = {
  "21:9": { width: 4608, height: 1968 },
  "16:9": { width: 4096, height: 2304 },
  "3:2": { width: 3984, height: 2656 },
  "4:3": { width: 3648, height: 2736 },
  "1:1": { width: 3328, height: 3328 },
  "3:4": { width: 2736, height: 3648 },
  "2:3": { width: 2656, height: 3984 },
  "9:16": { width: 2304, height: 4096 },
};

// 4K 对应的 ratio 值（与 1K 不同）
const RATIO_VALUES_4K: Record<string, number> = {
  "21:9": 100,
  "16:9": 101,
  "3:2": 102,
  "4:3": 103,
...
export async function generateImages(
  ...
  } else if (!["1k", "2k", "4k"].includes(resolutionType)) {
    // 如果未指定或不明确，根据模型默认
    resolutionType = is4xModel ? "2k" : "1k";
  }

  const dimensionMap = resolutionType === "4k" ? DIMENSIONS_4K : resolutionType === "2k" ? DIMENSIONS_2K : DIMENSIONS_1K;
  ...
  const imageRatio = resolutionType === "4k" ? (RATIO_VALUES_4K[validRatio] ?? RATIO_VALUES[validRatio]) : RATIO_VALUES[validRatio];
  ...
  const requestData = {
    ...
    draft_content: JSON.stringify({
      ...
      min_features: [],
      is_from_tsn: true,
      version: getDraftVersion(modelName),
  ...
  const { aigc_data } = await request(
    ...
    {
      params: {
        da_version: getDraftVersion(modelName),
        web_component_open_flag: 1,
        web_version: getDraftVersion(modelName),
      },
      data: requestData,
    }
  );
...
export async function generateImagesWithRetry(
  ...
  const resolutionLevels = ["4k", "2k", "1k"]; // 分辨率降级顺序
...
export async function generateImageComposition(
  ...
  const model = getModel(_model) || getModel(DEFAULT_MODEL);
  const draftVersion = getDraftVersion(_model);
  const imageCount = imageUrls.length;

  // 解析比例和尺寸
  const ratioKey = ASPECT_RATIOS.includes(ratio) ? ratio : "1:1";
  const dimensionMap = resolution === "4k" ? DIMENSIONS_4K : resolution === "2k" ? DIMENSIONS_2K : DIMENSIONS_1K;
  const dimensions = dimensionMap[ratioKey] || dimensionMap["1:1"];
  const imageRatio = resolution === "4k" ? (RATIO_VALUES_4K[ratioKey] ?? RATIO_VALUES[ratioKey]) : RATIO_VALUES[ratioKey];
  ...
  for (let i = 0; i < imageUrls.length; i++) {
    try {
      const uploadResult = await uploadFile(refreshToken, imageUrls[i]);
      if (uploadResult && uploadResult.image_uri) {
        uploadedImageIds.push(uploadResult.image_uri);
      }
    } catch (error) {
      throw new APIException(EX.API_IMAGE_GENERATION_FAILED, `图片 ${i + 1} 上传失败: ${error.message}`);
    }
  }

  logger.info(`图生图: 已上传 ${uploadedImageIds.length} 张图片，开始合成`);
          ...
          component_list: [
            {
              ...
              abilities: {
                ...
                blend: {
                  ...
                  prompt_placeholder_info_list: uploadedImageIds.map(
                    (_, index) => ({
                      type: "",
                      id: util.uuid(),
                      ability_index: index,
  ...
  const historyId = aigc_data?.history_record_id;
  if (!historyId)
    throw new APIException(EX.API_IMAGE_GENERATION_FAILED, "图生图记录ID不存在");
  ...
  while (retryCount < maxRetries) {
    const result = await request("post", "/mweb/v1/get_history_by_ids", refreshToken, {
      data: {
        ...
        image_info: {
          ...
          format: "webp",
          image_scene_list: [
            { scene: "normal", width: dimensions.width, height: dimensions.height, uniq_key: String(dimensions.width), format: "webp" },
          ],
        },
    ...
    const historyData = result.history_list ? result.history_list[0] || result[historyId] : result[historyId];
    if (!historyData) {
      retryCount++;
    ...
    if (status === 30) {
      throw new APIException(EX.API_IMAGE_GENERATION_FAILED, `图生图失败: ${historyData.fail_code}`);
    }
    ...
    if (itemList.length > 0) {
      const urls = itemList
        .map((item: any) => {
          ...
          return common.cover_url || common.origin_url || item.url;
        })
        .filter(Boolean);
  ...
  throw new APIException(EX.API_IMAGE_GENERATION_FAILED, "图生图超时");
}
