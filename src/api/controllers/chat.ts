...
import { generateVideoWithRetry, DEFAULT_MODEL as DEFAULT_VIDEO_MODEL } from "./videos.ts";
...
function isVideoModel(model: string) {
  return model.startsWith("jimeng-video") || model.includes("seedance");
}
...
export async function createCompletion(
  ...
  return (async () => {
    ...
    if (isVideoModel(_model)) {
      try {
        // Seedance 模型通过 chat 接口时返回使用说明
        if (_model.includes("seedance")) {
          return {
            id: util.uuid(),
            model: _model,
            ...
            choices: [{
              ...
              message: {
                role: "assistant",
                content: `Seedance 2.0 需要通过专用视频接口使用。\n\n请使用 POST /v1/videos/generations：\n\n\`\`\`json\n{\n  "model": "seedance-2.0",\n  "prompt": "描述视频内容",\n  "ratio": "16:9",\n  "duration": 5,\n  "file_paths": ["图片URL（可选）"]\n}\`\`\`\n\n支持的模型：seedance-2.0（专业版）、seedance-2.0-fast（快速版）\n时长范围：4-15秒`,
              },
              finish_reason: "stop",
            }],
  ...
  return (async () => {
    ...
    if (isVideoModel(_model)) {
      ...
      if (_model.includes("seedance")) {
        stream.write(
          ...
          JSON.stringify({
            ...
            choices: [{
              index: 0,
              delta: { role: "assistant", content: "Seedance 2.0 需要通过 POST /v1/videos/generations 专用接口使用，不支持通过 chat 接口生成。请参阅 API 文档。" },
              finish_reason: "stop",
            }],
          }) + "\n\n"
        );
      ...
      generateVideoWithRetry(
        _model,
        promptText || lastMessage.content,
        { ratio: "16:9", resolution: "720p", duration: 5, filePaths: imageUrls },
        refreshToken
      )
        .then((videoUrl) => {
          clearInterval(progressInterval);
